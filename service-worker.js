  ...
  e.waitUntil(
    caches.open('moosteakdeveloper').then(cache => {
      return cache.addAll([
        ...
        `/static/js/manifest.js.json?timestamp=${timeStamp}`,
...
self.addEventListener('notificationclick', (event) => {
  ...
  if (event.action === 'confirmAttendance') {
    clients.openWindow((typeof eventURL.confirm === 'undefined') ? 'https://moosteakdeveloper.firebaseapp.com' : eventURL.confirm);
  } else {
    clients.openWindow((typeof eventURL.decline === 'undefined') ? 'https://moosteakdeveloper.firebaseapp.com' : eventURL.decline);
  }
}, false);
...
messaging.setBackgroundMessageHandler((payload) => {
  ...
  const notificationOptions = {
    ...
    // actions: [
    //   {action: 'confirmAttendance', title: 'ยอมรับ'},
    //   {action: 'cancel', title: 'ยกเลิก'}
    // ],
    // data: {confirm: data.confirm, decline: data.decli111ne}
  };
